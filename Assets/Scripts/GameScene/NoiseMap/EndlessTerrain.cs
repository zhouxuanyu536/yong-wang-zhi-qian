...
public class EndlessTerrain : MonoBehaviour {
    ...
    private void Update()
    {
        if (PlayerTools.GetLocalPlayer() != null) 
            viewer = PlayerTools.GetLocalPlayer();
        else
        {
            viewer = GameObject.Find("defaultViewer").transform;
        }
        viewerPosition = new Vector2(viewer.position.x,viewer.position.z) / mapGenerator.terrainData.uniformScale;

        if((viewerPositionOld - viewerPosition).sqrMagnitude > sqrViewerMoveThresholdForChunkUpdate) //区块更新
        {
